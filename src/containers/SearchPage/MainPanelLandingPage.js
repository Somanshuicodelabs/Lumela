...
import TopbarSearchForm from '../SearchPage/TopbarSearchForm/TopbarSearchForm';
...
class MainPanelLanding extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  handleSubmit(values) {
    ...
    const originMaybe = searchfilter.sortSearchByDistance ? { origin } : {};
    const searchParams = {
      ...currentSearchParams,
      ...originMaybe,
      address: search,
  ...
  render() {
    ...
    const landingfilter = filterConfig.filter(e => e.id != 'dates-length');

    const primaryFilters = landingfilter.filter(f => f.group === 'primary');
    // console.log('aaaprimaryFilters :>> ', primaryFilters);
    const secondaryFilters = filterConfig.filter(f => f.group !== 'primary');
    // console.log('secondaryFilters :>> ', secondaryFilters);
    ...
    const classes = classNames(rootClassName || css.searchContainerLanding, className);

    return (
      <>
        <div className={classes}>
          <SearchFiltersPrimary
            className={css.landingSearchFiltersPrimary}
          ...
          <TopbarSearchForm
            ...
            onSubmit={e => {
              this.setState({ currentQueryParams: { ...this.state.currentQueryParams, ...e } });
            }}
          ...
          <Button
            onClick={() => {
              if (this.state.currentQueryParams.location) {
                ...
                this.state.currentQueryParams
                const querySearch = this.state.currentQueryParams?.keywords?.length ? { keywords: this.state.currentQueryParams?.keywords } : {}
                history.push(
        ...
        <div className={css.areYouBusiness}>
          Are you a business?{' '}
          <a href="/">
            {viewport.width && viewport.width < 768 ? 'Add a listing' : 'Click here to a listing!'}
          </a>
        </div>
      </>
    );
...
const EnhancedMainPanelLandingPage = props => {
  const config = useConfiguration();
...
const MainPanelLandingPage  = withConfiguration(MainPanelLanding)
